...
def mouse_gray(event, x, y, flags, params):
    ...
    if event == cv2.EVENT_LBUTTONDOWN:
        if choice == 49:
            ...
            points = np.array([[x-20, y-20], [x+20, y-20], [x+30, y], [x, y+20], [x-30, y]], np.int32)
...
def mouse_rgb(event, x, y, flags, params):
    ...
    if event == cv2.EVENT_LBUTTONDOWN:
        if choice == 49:
            ...
            points = np.array([[x-20, y-20], [x+20, y-20], [x+30, y], [x, y+20], [x-30, y]], np.int32)
            cv2.fillPoly(rgb_image, [points], (255), cv2.LINE_AA)
            cv2.imshow("RGB", rgb_image)
...
while True:
    ...
    if choice == 81 or choice == 113:
        ...
        print("Pentagon : select CENTER with LEFT MOUSE BUTTON")

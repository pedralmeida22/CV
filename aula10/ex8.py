...
if len(image.shape) < 3:
    print("The loaded image is NOT a RGB image !")
    exit(-1)
...
hist_b = compute_histogram(blue, histSize, histRange)
hist_g = compute_histogram(green, histSize, histRange)
hist_r = compute_histogram(red, histSize, histRange)

# DISPLAY THE HISTOGRAM USING matplotlib
# azul == preto
plt.plot(hist, drawstyle='steps-post', color='black')
plt.plot(hist_b, drawstyle='steps-post', color='blue')
plt.plot(hist_g, drawstyle='steps-post', color='green')
plt.plot(hist_r, drawstyle='steps-post', color='red')
plt.xlim(histRange)
plt.title("Histogram")
plt.show()
